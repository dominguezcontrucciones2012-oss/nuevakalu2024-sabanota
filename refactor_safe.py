...
# 4. State
content = content.replace("const [lastReceipt, setLastReceipt] = useState<any | null>(null);", 
"""const [lastReceipt, setLastReceipt] = useState<any | null>(null);
  const [isPaymentHubOpen, setIsPaymentHubOpen] = useState(false);
  const [printReceipt, setPrintReceipt] = useState(true);""")

# 5. Customer Type Selector (Remove General)
old_customer_selector = """{[
                      { type: 'general', label: 'General' },
                      { type: 'client', label: 'Cliente' },
                      { type: 'supplier', label: 'Libreta Quesero' }
                    ]"""
new_customer_selector = """{[
                      { type: 'client', label: 'Cliente' },
                      { type: 'supplier', label: 'Libreta Quesero' }
                    ]"""
content = content.replace(old_customer_selector, new_customer_selector)
...
# 7. Remove payment form part
old_payment_section_start = "{/* Payment Method Select */}"
old_payment_section_end = ")} </div> </div> )}" # Actually it ends with a closing tag
# Let's just find the exact block using regex carefully without matching everything
# We'll use split
parts = content.split("{/* Payment Method Select */}")
if len(parts) == 2:
    part2 = parts[1]
    subparts = part2.split("<div className=\"h-px bg-editorial-border/60\" />")
    if len(subparts) >= 2:
        content = parts[0] + "<div className=\"h-px bg-editorial-border/60\" />" + subparts[1]
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-[400px] h-full bg-editorial-card border-l border-editorial-border shadow-2xl flex flex-col p-6 animate-slide-left rounded-none">
            ...
            <div className="flex-1 space-y-6 overflow-y-auto">
              ...
              <div className="flex items-center gap-3 py-4 border-y border-editorial-border/30 mt-4">
                <input 
                  type="checkbox" 
