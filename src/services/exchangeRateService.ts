export async function fetchOfficialBcvRate(): Promise<{ rate: number; timestamp: string }> {
  try {
    ...
    const res = await fetch('https://ve.dolarapi.com/v1/dolares/oficial', { signal: controller1.signal });
    ...
    try {
      ...
      const fallbackRes = await fetch('https://pydolarvenezuela-api.vercel.app/api/v1/dollar?page=bcv', { signal: controller2.signal });
