...
export default function PaymentsTab({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 relative bg-zinc-950 pb-24 animate-fade-in text-white overflow-y-auto w-full max-w-full">

      {/* 1. Header Resumen de Deuda */}
      <div className="p-4 sm:p-5 bg-gradient-to-b from-emerald-950/60 via-zinc-950/90 to-zinc-950 border-b border-zinc-900">
        <div className="flex justify-between items-center mb-1">
          <h2 className="text-lg sm:text-xl font-black tracking-tight">Pagos y Deudas</h2>
          <span className="text-[10px] text-zinc-400 font-mono bg-zinc-900 px-2 py-1 rounded-full border border-zinc-800">
        ...
        <div className="bg-zinc-900/90 border border-emerald-500/30 rounded-3xl p-5 shadow-[0_0_30px_rgba(16,185,129,0.08)] relative overflow-hidden text-center">
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-emerald-500/10 blur-3xl rounded-full"></div>
          <p className="text-[11px] font-bold text-emerald-400 uppercase tracking-widest mb-1.5 flex items-center justify-center gap-1.5">
          ...
          <div className="space-y-3">
            ...
            {activeGroups.map((group) => {
              ...
              return (
                <div
                  key={group.groupKey}
                  className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-2xl overflow-hidden shadow-sm transition-all"
                >
                  ...
                  <button
                    ...
                    className="w-full p-4 flex justify-between items-center text-left cursor-pointer hover:bg-zinc-800/40 transition-colors min-h-[56px] focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
                  >
      ...
      {showPaymentModal && (() => {
        ...
        return (
          <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col animate-in slide-in-from-bottom duration-300 overflow-y-auto w-full">
            {/* Header Modal */}
            <div className="sticky top-0 z-50 bg-slate-950/90 backdrop-blur-md border-b border-slate-800 p-4 flex justify-between items-center">
              ...
              <button
                ...
                className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center text-slate-400 hover:text-white transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-5 space-y-5 pb-36 max-w-lg mx-auto w-full">
              ...
              <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 space-y-1 text-xs">
                <span className="text-[10px] text-slate-400 uppercase font-bold block">Destino del Pago</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-3 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-1 min-h-[56px] cursor-pointer ${
                      selectedBank === '0102' ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-slate-800 bg-slate-900 text-slate-400'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`p-3 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-1 min-h-[56px] cursor-pointer ${
                      selectedBank === '0134' ? 'border-emerald-500 bg-emerald-500/10 text-white' : 'border-slate-800 bg-slate-900 text-slate-400'
                    }`}
                  >
              ...
              <div className="space-y-2.5">
                ...
                <div
                  ...
                  className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex justify-between items-center active:scale-[0.99] transition-transform cursor-pointer min-h-[48px]"
                >
                  <div>
                    <p className="text-[9px] text-slate-500 uppercase font-bold">Teléfono Receptor</p>
                    <p className="text-sm font-mono text-white font-bold">{phoneStr}</p>
                  </div>
                  {copiedPhone ? <Check className="w-5 h-5 text-emerald-400" /> : <Copy className="w-4 h-4 text-slate-500" />}
                </div>
                ...
                >
                  ...
                  {copiedId ? <Check className="w-5 h-5 text-emerald-400" /> : <Copy className="w-4 h-4 text-slate-500" />}
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                ...
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-lg font-black text-emerald-500 font-mono">
                    Bs.
                  </span>
                  <input
                    type="number"
                    ...
                    className="w-full bg-slate-900 border-2 border-slate-800 rounded-2xl py-3.5 pl-12 pr-12 text-2xl font-black text-white font-mono focus:outline-none focus:border-emerald-500 transition-colors"
                  />
                  <button
                    type="button"
                    onClick={() => handleCopy(paymentAmountBs, setCopiedAmount)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white p-2 min-h-[44px]"
                  >
              ...
              <div className="space-y-3 pt-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">
                  4. Comprobante Bancario <span className="text-emerald-400">*</span>
                </label>
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border-2 border-slate-800 rounded-xl py-3 px-4 text-base font-mono text-white focus:outline-none focus:border-emerald-500 transition-colors placeholder:text-slate-600 min-h-[48px]"
                />
                ...
                <div className="relative">
                  {imagePreview ? (
                    ...
                    <label className="border-2 border-dashed border-emerald-500/30 rounded-2xl p-5 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500/70 hover:bg-slate-900/50 transition-colors min-h-[100px]">
                      <ImageIcon className="w-7 h-7 text-emerald-400 mb-1.5" />
                      <span className="text-xs font-bold text-slate-200">Adjuntar Comprobante de Pago (Obligatorio)</span>
                      <span className="text-[9px] text-slate-400 mt-0.5">JPG, PNG o WEBP (Máx. 4MB)</span>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-5 bg-gradient-to-t from-slate-950 via-slate-950 to-transparent border-t border-slate-900/80 max-w-lg mx-auto w-full">
              <button
                type="button"
                disabled={isSubmitting || !paymentAmountBs || !reference || Number(paymentAmountBs) <= 0 || reference.length < 4 || !imagePreview}
                onClick={handleSubmitPayment}
                className="w-full py-3.5 sm:py-4 bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-800 disabled:text-slate-600 text-slate-950 font-black uppercase rounded-2xl text-sm tracking-widest transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] disabled:shadow-none flex items-center justify-center gap-2 cursor-pointer min-h-[48px]"
              >
