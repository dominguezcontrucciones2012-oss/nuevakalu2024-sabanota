...
interface ClientsCreditViewProps {
  ...
  exchangeRate?: number;
  salesHistory?: any[];
  onAddClient: (client: Omit<ClientProfile, 'id' | 'outstandingDebt'> & { loyaltyPoints?: number }) => void;
...
export default function ClientsCreditView({
  ...
  // Comprobantes / Captures para el historial de clientes
  ...
  useEffect(() => {
    // Escuchar Pagos PWA para resolver comprobantes / captures en el historial
  ...
  const [tier, setTier] = useState<string>('K1');
  ...
  const [editTier, setEditTier] = useState<string>('K1');
  ...
  const openEditModal = (c: ClientProfile) => {
    const vip = getVIPLevelInfo(c.loyaltyPoints || 0);
    setEditingClient(c);
    setEditName(c.name || '');
    ...
    setEditPin(c.pin || '');
    setEditTier(c.tier || vip.code);
    setEditLoyaltyPoints(c.loyaltyPoints || 0);
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showAddForm && (
        <form onSubmit={handleCreateClient} className="bg-editorial-card border border-editorial-border rounded p-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-mono text-editorial-text-muted uppercase block">Nivel / Rango VIP</label>
            <select
              value={tier}
              onChange={e => {
                const selectedCode = e.target.value;
                setTier(selectedCode);
                const found = VIP_LEVELS_MATRIX.find(m => m.code === selectedCode);
                if (found) setLoyaltyPoints(found.minPoints);
              }}
            ...
            >
              {VIP_LEVELS_MATRIX.map(lvl => (
                <option key={lvl.code} value={lvl.code}>
                  {lvl.code} - {lvl.name} (Cupo ${lvl.mainCreditLimit} | Inicial {Math.round(lvl.initialPct * 100)}%)
                </option>
              ))}
            </select>
          </div>
                ...
                <div key={c.id} className="bg-editorial-card border border-editorial-border rounded p-5 flex flex-col justify-between hover:border-amber-500/40 transition-all duration-300">
                  <div className="space-y-3">
                    <div className="flex justify-between items-start">
                      <span className={`text-[9px] font-mono font-extrabold uppercase px-2.5 py-1 rounded-full border shadow-xs bg-gradient-to-r ${vip.badgeColor}`}>
                        {vip.code} • {vip.shortName}
                      </span>
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-1 mt-2 text-[11px] text-editorial-text-muted">
                        <div className="flex items-center gap-1.5">
                          <Phone className="w-3.5 h-3.5 shrink-0 text-amber-400/80" />
                          <a href={`tel:${c.phone}`} className="hover:text-amber-500 transition-colors">{c.phone || 'Sin número'}</a>
                        </div>
                        {(c.cedula || c.rfc) && (
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] font-bold text-editorial-text-muted/70">C.I / RIF:</span>
                            <span className="font-mono text-editorial-text-primary">{c.cedula || c.rfc}</span>
                          </div>
                        )}
                        {c.address && (
                          <div className="flex items-start gap-1.5 mt-0.5">
                            <span className="text-[10px] font-bold text-editorial-text-muted/70">Ubic:</span>
                            <span className="truncate">{c.address}</span>
                          </div>
                        )}
                        {c.birthday && (
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] font-bold text-editorial-text-muted/70">Cump:</span>
                            <span>{c.birthday}</span>
                          </div>
                        )}
                        {c.email && (
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <Mail className="w-3.5 h-3.5 shrink-0 text-amber-400/80" />
                    ...
                    {/* VIP Level Perks Info */}
                    <div className="p-2 rounded bg-editorial-bg/50 border border-editorial-border/60 text-[10px] font-mono text-editorial-text-muted flex justify-between items-center">
                      <span>Línea: <strong className="text-editorial-text-primary">${vip.mainCreditLimit}</strong> ({vip.mainMaxInstallments}c)</span>
                      <span>Inicial: <strong className="text-emerald-400">{Math.round(vip.initialPct * 100)}%</strong></span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-editorial-border/60 grid grid-cols-2 gap-2 text-center font-mono">
                    ...
                    <div>
                      ...
                      <div className={`text-xs font-bold mt-0.5 ${hasDebt ? 'text-rose-400' : 'text-emerald-400'}`}>
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-editorial-border text-[10px] font-mono text-editorial-text-muted uppercase tracking-wider">
                    ...
                    <th className="py-3 px-3 text-center">Nivel VIP</th>
                    ...
                    clientsWithDebt.map((c) => {
                      ...
                      <tr key={c.id} className="hover:bg-editorial-bg/30 transition-all">
                        ...
                        <td className="py-3 px-3 text-center">
                          <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold border bg-gradient-to-r ${vip.badgeColor}`}>
                            {vip.code} - {vip.shortName}
                          </span>
                        </td>
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-editorial-card border border-editorial-border rounded-lg max-w-2xl w-full p-6 relative shadow-2xl">
            ...
            <form onSubmit={handleUpdateClientSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    setEditLoyaltyPoints(pts);
                    const calculated = getVIPLevelInfo(pts);
                    setEditTier(calculated.code);
                  }}
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-mono text-editorial-text-muted uppercase block">Nivel / Matriz VIP</label>
                <select
                  value={editTier}
                  onChange={(e: any) => {
                    const selectedCode = e.target.value;
                    setEditTier(selectedCode);
                    const found = VIP_LEVELS_MATRIX.find(m => m.code === selectedCode);
                    if (found) setEditLoyaltyPoints(found.minPoints);
                  }}
                ...
                >
                  {VIP_LEVELS_MATRIX.map(lvl => (
                    <option key={lvl.code} value={lvl.code}>
                      {lvl.code} - {lvl.name} (${lvl.mainCreditLimit})
                    </option>
                  ))}
                </select>
              </div>
