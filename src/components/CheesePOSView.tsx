...
export default function CheesePOSView({
  ...
  const totalAbonado = Math.round(rawAbonado * 100) / 100;
  const remainingDebt = Math.max(0, Math.round((total - totalAbonado) * 100) / 100);
  // Blindaje con tolerancia de 2 céntimos USD (0.02) o residuo menor a 0.02 USD
  const isFullyPaid = remainingDebt <= 0.02 || (totalAbonado >= total - 0.02);
  ...
  const handleProcessSaleSubmit = async (e?: React.FormEvent, bypassChangeModal = false) => {
    ...
    if (isSubmittingRef.current || isProcessing || (now - lastSubmitTimeRef.current < 800)) {
      console.warn('[POS] Bloqueo antirrebote: Intento de envío duplicado descartado.');
      return;
    }
    ...
    if (currentPayments.length === 0 && !isCreditSale && paymentMethod !== 'Mundo Kalu') {
      ...
      if (rawInput > 0) {
        ...
        currentPayments.push(autoPayment);
      }
    }
    ...
    if (currentPayments.length === 1) {
      mainPaymentMethod = currentPayments[0].method;
    ...
    if (!isEffectiveCredit && !effectiveFullyPaid) {
    ...
    const paidAmount = isEffectiveCredit ? currentTotalAbonado : Math.max(total, currentTotalAbonado);
    const debtAmount = isEffectiveCredit ? currentRemainingDebt : 0;

    let mainReference = currentPayments.map(p => p.reference).filter(Boolean).join(' | ');

    // Calculate change logic
    const changeAmount = (currentTotalAbonado - total > 0.0099) ? parseNum((currentTotalAbonado - total).toFixed(2)) : 0;
                ...
                {customerType === 'client' && (
                  <div className="space-y-1.5 relative">
                    <div className="flex justify-between items-center">
                      <label className="text-[10px] font-mono tracking-wider text-editorial-text-muted uppercase block">
                        Cliente de Tienda (Opcional / Mostrador)
                      </label>
                      {selectedClientId && (
                        <button
                          type="button"
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        placeholder="Buscar cliente (o dejar vacío para Mostrador)..."
                      ...
                      {clientSearchText && (
                        <button
                          ...
                          className="absolute right-2.5 top-1/2 -translate-y-1/2 text-editorial-text-muted hover:text-rose-400 text-xs"
                        >
                          &times;
                        </button>
                      )}
                      {isClientDropdownOpen && (
                        <div className="absolute z-20 w-full mt-1 bg-editorial-card border border-editorial-border rounded shadow-2xl max-h-56 overflow-y-auto">
                          <div
                            onMouseDown={() => {
                              setSelectedClientId('');
                              setClientSearchText('');
                            ...
                            className="px-4 py-2 hover:bg-editorial-bg cursor-pointer text-xs transition-colors border-b border-editorial-border/40 font-mono text-editorial-text-muted italic"
                          >
                            👤 Cliente de Mostrador (General)
                          </div>
                          {clients.filter(c => {
                            ...
                            <div
                              ...
                              className={`px-4 py-2 hover:bg-amber-500 hover:text-white cursor-pointer text-xs transition-colors border-b border-editorial-border/30 last:border-0 ${
                                selectedClientId === c.id ? 'bg-amber-500/20 text-amber-300 font-bold' : ''
                              }`}
                            >
                              <div className="font-semibold">{c.name}</div>
                              <div className="text-[10px] text-editorial-text-muted">
                                {c.rfc || c.ci || c.cedula || c.ciRif || 'S/N'}
                                { (c.outstandingDebt || 0) > 0 && (
                                  <span className="text-rose-400 font-bold ml-2">Deuda: ${(c.outstandingDebt || 0).toFixed(2)}</span>
                                )}
                              </div>
                            </div>
                          ))}
                        ...
                        <div className="bg-editorial-bg border border-editorial-border rounded p-4 space-y-3">
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <div className="text-right">
                              ...
                              <div className="font-mono text-[10px] text-editorial-text-muted">Bs {(total * activeExchangeRate).toFixed(2)}</div>
                            </div>
                          </div>
                          <div className="flex justify-between items-center text-sm">
                          ...
                          {totalAbonado - total > 0.0099 ? (
                            <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded animate-in fade-in duration-300">
                              <div className="flex justify-between items-center text-sm mb-1">
                                ...
                                <div className="text-right">
                                  ...
                                  <div className="font-mono text-[10px] text-amber-500/70">Bs {((totalAbonado - total) * activeExchangeRate).toFixed(2)}</div>
                                </div>
                              </div>
                            </div>
                          ) : isFullyPaid ? (
                            ...
                            <div className="p-3 bg-cyan-500/10 border border-cyan-500/30 rounded animate-in fade-in duration-300">
                              <div className="flex justify-between items-center text-sm mb-1">
                                ...
                                <div className="text-right">
                                  <div className="font-mono text-2xl font-bold text-cyan-400">${remainingDebt.toFixed(2)}</div>
                                  <div className="font-mono text-[10px] text-cyan-400/70">Bs {(remainingDebt * activeExchangeRate).toFixed(2)}</div>
                                </div>
                              </div>
                            </div>
                          ) : (
                            <div className="flex justify-between items-center text-sm group cursor-pointer" onClick={() => {
                              const rem = remainingDebt;
                              setPaidAmountInput(paymentMethod === 'Efectivo $' ? rem.toFixed(2) : (rem * activeExchangeRate).toFixed(2));
                            }}>
                              ...
                              <div className="text-right">
                                <div className="font-mono text-2xl font-bold text-rose-400 group-hover:text-rose-300 transition-colors">${remainingDebt.toFixed(2)}</div>
                                <div className="font-mono text-[10px] text-rose-400/70 group-hover:text-rose-300/70 transition-colors">
                                  Bs {(remainingDebt * activeExchangeRate).toFixed(2)}
                                </div>
                              </div>
                            </div>
                          )}
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (m.id === 'Mundo Kalu' && customerType === 'client') {
                                    ...
                                    setPaymentReference('');
                                    // Autocompletar el restante en la moneda seleccionada con la tasa real activa
                                    setPaidAmountInput(m.id === 'Efectivo $' ? rem.toFixed(2) : (rem * activeExchangeRate).toFixed(2));
                                  }
                                }}
                                  ...
                                  <div className="flex justify-between items-center">
                                    ...
                                    {paymentMethod !== 'Efectivo $' && parseSafeDecimal(paidAmountInput) > 0 && (
                                      <span className="text-[10px] font-mono text-emerald-400 font-bold">
                                        ≈ ${(parseSafeDecimal(paidAmountInput) / activeExchangeRate).toFixed(2)} USD
                                      </span>
                                    )}
                                    {paymentMethod === 'Efectivo $' && parseSafeDecimal(paidAmountInput) > 0 && (
                                      <span className="text-[10px] font-mono text-emerald-400 font-bold">
                                        ≈ Bs {(parseSafeDecimal(paidAmountInput) * activeExchangeRate).toFixed(2)}
                                      </span>
                                    )}
                                  </div>
                                  <input
                                    ...
                                    placeholder={`Ej: ${paymentMethod === 'Efectivo $' ? Math.max(0, total - totalAbonado).toFixed(2) : (Math.max(0, total - totalAbonado) * activeExchangeRate).toFixed(2)}`}
