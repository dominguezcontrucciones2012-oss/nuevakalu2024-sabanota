    ...
    try:
        ...
        if sobrante_queso > 0 and saldo_anterior < 0:
            ...
            if cta_deuda_prod and deuda_a_cobrar > 0:
                ...
                # ✅ REGISTRAR COBRO DE DEUDA EN LIBRO DIARIO (Caja)
                # El productor canceló parte de su deuda con la entrega de queso.
                # Esto es un INGRESO "en especie" que debe aparecer en el libro diario
                # como cobro de cuenta por cobrar, para que el cuadre de ingresos sea correcto.
                db.session.add(MovimientoCaja(
                    fecha=datetime.now(),
                    tipo_caja='Cobro Queso',
                    tipo_movimiento='INGRESO',
                    categoria='Cobro Deuda Queso',
                    monto=deuda_a_cobrar,
                    tasa_dia=valor_tasa,
                    descripcion=f'Cobro deuda {productor.nombre}: {kilos}kg queso cancelan ${deuda_a_cobrar:.2f}',
