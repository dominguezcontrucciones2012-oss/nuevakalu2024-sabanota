...
class Producto(db.Model):
    ...
    codigo2 = db.Column(db.String(50), unique=True, nullable=True)
    codigo3 = db.Column(db.String(50), unique=True, nullable=True)
    codigo4 = db.Column(db.String(50), unique=True, nullable=True)
    nombre = db.Column(db.String(100), nullable=False)
    ...
    @validates('precio_oferta_usd', 'costo_usd')
    def validar_precio_oferta(self, key, value):
        costo = self.costo_usd
        precio_oferta = self.precio_oferta_usd

        if key == 'costo_usd':
            costo = value
        elif key == 'precio_oferta_usd':
            precio_oferta = value
            # Verificar si el precio de oferta está cambiando y validar permisos
            old_val = self.precio_oferta_usd
            old_decimal = Decimal(str(old_val)) if old_val is not None else Decimal('0.00')
            new_decimal = Decimal(str(value)) if value is not None else Decimal('0.00')
            ...
            if old_decimal != new_decimal:
                ...
                if has_request_context():
                    ...
                    try:
                        if current_user.is_authenticated and current_user.role not in ['admin', 'dueno']:
                            raise ValueError(
                                "Permiso denegado: Solo el Administrador o el Dueño pueden establecer o modificar precios de oferta."
                            )
                    except Exception as e:
                        if isinstance(e, ValueError) and "Permiso denegado" in str(e):
                            raise e
        ...
        if precio_oferta is not None:
            ...
            precio_oferta = Decimal('0.00')

        if precio_oferta > 0:
            minimo_oferta = (costo * Decimal('1.15')).quantize(Decimal('0.01'))
            if precio_oferta < minimo_oferta:
                raise ValueError(
                    f"Regla de ganancia (Mínimo 15%): El precio de oferta (${precio_oferta:.2f}) "
                    f"no puede ser menor a un 15% sobre el costo de compra (${costo:.2f}). "
                    f"El precio de oferta mínimo permitido es: ${minimo_oferta:.2f}"
                )
        return value
...
class Venta(db.Model):
    ...
    __table_args__ = (
        db.Index('idx_venta_fecha_pagada', 'fecha', 'pagada'),
        db.Index('idx_venta_fiado_pagada', 'es_fiado', 'pagada')
    )
    id = db.Column(db.Integer, primary_key=True)
    fecha = db.Column(db.DateTime, default=ahora_ve, index=True)
    ...
    def metodos_pago_activos(self):
        metodos = []
        if self.pago_efectivo_usd and self.pago_efectivo_usd > 0:
            metodos.append({'nombre': 'Efec $', 'clase': 'badge-success'})
        if self.pago_efectivo_bs and self.pago_efectivo_bs > 0:
            metodos.append({'nombre': 'Efec Bs', 'clase': 'badge-primary'})
        if self.pago_movil_bs and self.pago_movil_bs > 0:
            metodos.append({'nombre': 'P.Móvil', 'clase': 'badge-info'})
        if self.pago_transferencia_bs and self.pago_transferencia_bs > 0:
            metodos.append({'nombre': 'Transf', 'clase': 'badge-secondary'})
        if self.biopago_bdv and self.biopago_bdv > 0:
            metodos.append({'nombre': 'BioPago', 'clase': 'badge-dark'})
        if self.pago_debito_bs and self.pago_debito_bs > 0:
            metodos.append({'nombre': 'Débito', 'clase': 'badge-light text-dark'})
        if self.pago_otros_usd and self.pago_otros_usd > 0:
            metodos.append({'nombre': 'Otros $', 'clase': 'badge-warning'})
        return metodos
...
class MovimientoCaja(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    fecha = db.Column(db.DateTime, default=ahora_ve, index=True)
    tipo_caja = db.Column(db.String(30), nullable=False, index=True)
    tipo_movimiento = db.Column(db.String(10), nullable=False, index=True)
