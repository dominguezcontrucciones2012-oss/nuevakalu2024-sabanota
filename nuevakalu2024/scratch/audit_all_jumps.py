...
with app.app_context():
    ...
    output_path = os.path.join(os.path.dirname(__file__), 'audit_all_jumps_results.txt')
    
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("INICIANDO AUDITORIA CRONOLOGICA DE SALTOS DE SALDO...\n")
        f.write("=" * 80 + "\n")
        
        for p in productores:
            # Traer todos los movimientos en orden cronologico
            ...
            for m in movs:
                ...
                # El saldo del momento se convierte en la base para el siguiente movimiento cronologico
            ...
            if jumps_found:
                ...
                for j in jumps_found:
                    f.write(
                        f"  [SALTO] ID: {j['id']:4d} | Fecha: {j['fecha']} | Tipo: {j['tipo']:12s} |\n"
                        f"    Esperado: {j['expected']:8.2f} | Registrado: {j['actual']:8.2f} | Desviacion: {j['diff']:+8.2f} |\n"
                        f"    Desc: {j['desc']}\n"
                    )
                f.write("-" * 80 + "\n")
                
    print(f"Auditoria completada. Resultados guardados en: {output_path}")
